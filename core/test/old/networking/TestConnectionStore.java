...
public class TestConnectionStore {
  ...
  public void testGetAll() {
    ...
    //        assertEquals(c2,store.getAll(UpdateConnection.class).toArray(new
    // UpdateConnection[0])[1]);
    //        assertEquals(c3,store.getAll(UpdateConnection.class).toArray(new
    // UpdateConnection[0])[0]);
    assertThat(
        Arrays.asList(store.getAll(UpdateConnection.class).toArray(new UpdateConnection[0])),
        containsInAnyOrder(c2, c3));
  }
}

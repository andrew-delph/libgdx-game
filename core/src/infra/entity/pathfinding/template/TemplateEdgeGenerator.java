...
public class TemplateEdgeGenerator {
  ...
  TemplateEdgeGenerator(RelativeActionEdgeGenerator relativeActionEdgeGenerator,BlockStructureFactory blockStructureFactory,EdgeStore edgeStore,RelativeVertex rootRelativeVertex) {
    ...
    this.relativeActionEdgeList.add(this.relativeActionEdgeGenerator.generateRelativeActionEdge(rootBlockStructure,this.rootRelativeVertex,"jump"));
  }
  ...
  public void applyAction(String actionKey) {
    ...
    RelativeActionEdge newActionEdge = this.relativeActionEdgeGenerator.generateRelativeActionEdge(currentRelativeVertex.blockStructure,currentRelativeVertex,actionKey);

    this.relativeActionEdgeList.add(newActionEdge);

    if(lastActionEdge.getFrom().relativeCoordinates.equalBase(lastActionEdge.getTo().relativeCoordinates)){
      System.out.println(lastActionEdge.getFrom().relativeCoordinates+", "+lastActionEdge.getTo().relativeCoordinates);
      // add to the store
      TemplateEdge newTemplateEdge = new TemplateEdge(newActionEdge.getTo().blockStructure, this.rootRelativeVertex,currentRelativeVertex);
